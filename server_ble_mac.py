"""
Helios BLE Voice + Vision Server — MACOS BUILD
================================================
**This is the macOS dev build.** Use this on a MacBook to validate the
STT → LLM → TTS pipeline against a real ESP32 without depending on any
Linux/bluez quirks. Uses `bleak` (which works fine on macOS CoreBluetooth)
instead of the Rust `helios_ble` transport used on the Pi.

Primary use: isolate software bugs from hardware/driver quirks during dev.
For deployment on the Pi, use `server_ble.py` (helios_ble transport).
...
Dependencies: bleak, opuslib (requires libopus via brew), httpx, websockets, anthropic
Environment: .env file with CARTESIA_API_KEY, ANTHROPIC_API_KEY

Run:    python server_ble_mac.py
Debug:  python server_ble_mac.py --debug

macOS setup:
    brew install opus
    pip install bleak opuslib httpx websockets python-dotenv anthropic numpy fastapi uvicorn
"""
